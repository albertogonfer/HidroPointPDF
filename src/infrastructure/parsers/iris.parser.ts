...
/**
 * IRIS Assistance parser.
 */
export const irisParser: InvoiceParser = {
  companyId: 'IRIS',
...
function extractInvoiceNumber(text: string): string | null {
  const match = text.match(/(?:Nº?\s*(?:EXPEDIENTE|FACTURA|SINIESTRO)\.?\s*:?\s*)([\w/-]+)/i)
  if (match) return match[1].trim()
  const altMatch = text.match(/(?:EXPTE?\.?\s*:?\s*)([\w/-]+)/i)
...
function extractName(text: string): { firstName: string | null; lastName: string | null } {
  const match = text.match(/(?:ASEGURADO|CLIENTE|BENEFICIARIO)\s*:?\s*([A-ZÁÉÍÓÚÑÜ][a-záéíóúñü]+)\s+([A-ZÁÉÍÓÚÑÜ][a-záéíóúñü]+(?:\s+[A-ZÁÉÍÓÚÑÜ][a-záéíóúñü]+)?)/i)
